...
public class SearchControllerMockTest {
	...
	public void should_search() throws Exception {
		when(searchService.search1(anyString(), anyListOf(String.class))).thenReturn(Arrays.asList(new LightTweet("tweetText")));
		this.mockMvc.perform(get("/search/mixed;keywords=spring")).andExpect(status().isOk()).andExpect(view().name("resultPage"))
